...
Class Contact {
	...
	public function start ($receive=array()) {
		$base = array(
			'form'		=> '', // $_POST
			'file' 		=> '', // Campo file $_FILES['nome_campo']
			'subject' 	=> '', // Titulo Da mensagem
			'address' 	=> '', // Email destino
			'message' 	=> '', // Base das mensagens destino (trabalhe_conosco, contato)
			);

		self::$data = array_merge($base, $receive);
		self::$form = self::$data['form'];
		self::$file = self::$data['file'];

		if (!empty(self::$file) && self::$file['error'] == 0) {
			self::processFile();
		}
	...
	private function setMessage () {
		if (self::$data['message']=='trabalhe_conosco') {
			self::$message = '';
			self::$message .= "Nome: ".self::$form['nome']."\r\n";
			self::$message .= "Fone: ".self::$form['fone']."\r\n";
			self::$message .= "E-mail: ".self::$form['email']."\r\n";
			self::$message .= "Vaga Interece: ".implode(', ', self::$form['vaga_interece'])."\r\n";
			self::$message .= "Sobre a Pessoa: ".self::$form['sobre-voce']."\r\n";
			self::$message .= "\r\n";
		}
		elseif (self::$data['message']=='trabalhe_conosco') {}
	}
	...
	private function processFile () {
		...
		$avaExt = array('doc','docx','odt','pdf','txt'); // Avaliable extensions

		if ( in_array($ext, $avaExt) && (self::$file['size']/1024) < 200 ) {
			self::$fileName = $filename;
			self::$fileSend = self::$file['tmp_name'];
		}
	...
	private function processInput () {
		...
		$tempForm = self::$form;
		foreach ($tempForm AS $name => $value) {
	...
	private function sendEmail () {
		...
		$email->From      		= self::$form['email'];
		$email->FromName  		= self::$form['nome'];
		$email->Subject   		= "Trabalhe Conosco";
		$email->Body      		= self::$message;
		$email->AddAddress( 'guilherme@agenciaplanos.com.br' );

		// check if have an attachment
		if (!empty(self::$fileSend)) {
			$email->AddAttachment( self::$fileSend , self::$fileName );
		}
